...
export default function NavBar({ minimize, handleSideBar }: Props) {
    ...
    return (
        <div className="w-full h-[8%] flex border-b border-[#292929] items-center bg-[#030303]">
            ...
            <div className=''>
                <Image quality={100} className={`w-[80px] ${minimize ? 'mr-8' : 'mr-48'} transition-all duration-300 transform  h-full object-cover cursor-pointer text-white`} src='/youTubeMusic-icon.png' alt='youTubeMusic-icon' width={3000} height={912} />
            </div>
            <button>
                <IoSearchOutline size={24} className='text-[#DFDFDF] mr-3' />
            </button>
            ...
            <div className='flex hover:bg-white hover:bg-opacity-10 rounded-full focus:outline-none transition duration-150  p-1.5  ml-auto mr-4 cursor-pointer items-center'>
                <PiScreencastLight className='text-[#FFFFFF]' size={26} />
            </div>
            <Image quality={100} className='w-8 rounded-full h-8 mr-2   object-cover cursor-pointer' src='/userImage.jpg' alt='userImage' width={360} height={360} />
        </div>
    );
}
